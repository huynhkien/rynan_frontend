import { CustomTooltipProps, OrderStatusData } from '@/features/admin/type/adminType';
...
export const AdminManagementStatisticalOrder = ({ 
  orderData 
}:  {orderData: OrderData[]}) => {
  ...
  }, [orderData, selectedMonth, selectedYear]);

  const orderStatusData: OrderStatusData[] = [
    { name: 'Đang xử lý', value: filteredOrderData.filter(el => el.status === 'Processing').length || 0, total: filteredOrderData.filter(el => el.status === 'Processing').reduce((sum, item) => sum + item.total, 0) || 0, color: '#FFA726' },
    { name: 'Đang giao hàng', value: filteredOrderData.filter(el => el.status === 'Delivering').length || 0, total: filteredOrderData.filter(el => el.status === 'Delivering').reduce((sum, item) => sum + item.total, 0) || 0, color: '#42A5F5' },
    { name: 'Đã hủy', value: filteredOrderData.filter(el => el.status === 'Cancelled').length || 0, total: filteredOrderData.filter(el => el.status === 'Cancelled').reduce((sum, item) => sum + item.total, 0) || 0, color: '#EF5350' },
    { name: 'Đã nhận', value: filteredOrderData.filter(el => el.status === 'Received').length || 0, total: filteredOrderData.filter(el => el.status === 'Received').reduce((sum, item) => sum + item.total, 0) || 0, color: '#66BB6A' },
    { name: 'Giao hàng thành công', value: filteredOrderData.filter(el => el.status === 'Succeed').length || 0, total: filteredOrderData.filter(el => el.status === 'Succeed').reduce((sum, item) => sum + item.total, 0) || 0, color: '#26C6DA' },
  ];

  const totalOrderCount = filteredOrderData.length || 0;
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <Paper elevation={3} sx={{ p: 2, borderRadius: 2, border: `2px solid ${payload[0].payload.color}` }}>
          ...
          <Typography variant='body1' sx={{ color: 'text.secondary' }}>Số lượng: {payload[0].value} đơn</Typography>
          <Typography variant='body1' sx={{ color: 'text.secondary' }}>Tỷ lệ: {percentage}%</Typography>
          <Typography variant='body1' sx={{ color: 'text.secondary' }}>Tổng: {payload[0].payload.total.toLocaleString()} VNĐ</Typography>
        </Paper>
      );
    }
    return null;
              ...
              {orderStatusData.map((item, index) => (
                <Box key={index} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', p: 1.5, borderRadius: 1, backgroundColor: 'rgba(255, 255, 255, 0.6)', transition: 'all 0.3s ease', '&:hover': { backgroundColor: 'rgba(255, 255, 255, 0.8)', transform: 'translateX(4px)' } }}>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', textAlign: 'right' }}>
                    ...
                    <Typography variant='caption' color='text.secondary'>{item.total.toLocaleString()} VNĐ</Typography>
                  </Box>
                </Box>
              ))}
            </Box>
