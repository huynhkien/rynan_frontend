...
export const DecentralizeManagementFormAddEdit = () => {
    ...
    const handleGenerateCode = useCallback(() => {
        if (user.length === 0) {
            setValue('code', 'RSA-001');
            setLastCodeNumber(1);
            return;
        }
        
        let newNumber = lastCodeNumber + 1;
        let newCode = `RSA-00${newNumber}`;
        
        while (user.some((el) => el.code === newCode)) {
            newNumber += 1;
            newCode = `RSA-00${newNumber}`;
        }
    ...
    // Xử lý thêm thông tin nhân viên
        ...
        <Paper sx={{ mb: 2, p: 2, borderRadius: 0, backgroundColor: theme.palette.background.paper }}>
            <Box sx={{ py: 2 }}>
                <Typography variant='h6' sx={{ color: theme.palette.primary.main }}>
                    {id ? 'Cập nhật thông tin nhân viên' : 'Thêm thông tin nhân viên'}
                </Typography>
            </Box>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%', gap: 5 }}>
                    <Box sx={{ width: '50%' }}>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                            ...
                            {/* Loại nhân viên và Nguồn nhân viên */}
                            <UserFormInput
                                    label='Tên nhân viên'
                                    important
                                    ...
                                    id='name'
                                    validate={{ required: 'Tên nhân viên không được để trống' }}
                                />
                            ...
                            >
                                <UserFormInput
                                    label='Mã nhân viên'
                                    important
                                    placeholder='Thêm mã nhân viên'
                                    register={register as UseFormRegister<UserDataProps>}
                                    errors={errors as FieldErrors<UserDataProps>}
                                    id='code'
                                    validate={{ required: 'Mã nhân viên không được để trống' }}
                                    sx={{
                                        width: '70%'
                                    }}
                                />
                            ...
                            <ControlledSelect
                                ...
                                control={control}
                                options={Role}
                                rules={{ required: 'Vui lòng chọn quyền cho nhân viên' }}
                            />
                    ...
                    <Box sx={{ width: '50%', display: 'flex', flexDirection: 'column', gap: 1 }}>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                            ...
                            />
                            <Typography variant='body2' sx={{color: theme.palette.primary.main, my:2}}>Thông tin liên hệ nhân viên </Typography>
                            <UserFormInput
                                label='Số điện thoại'
                                important
