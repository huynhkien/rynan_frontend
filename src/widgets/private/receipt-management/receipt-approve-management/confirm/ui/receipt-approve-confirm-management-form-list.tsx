...
export const ReceiptApproveConfirmManagementFormList = () => {
    const theme = useTheme();
    ...
    const receiptsImportProduct = useMemo(() => {
      return receiptsImport?.filter((receipt) => 
        receipt.products && receipt.products.length > 0 && receipt.status === 'confirmed'
      ) || [];
    }, [receiptsImport]);
    const receiptsImportMaterial = useMemo(() => {
      return receiptsImport?.filter((receipt) => 
        receipt.materials && receipt.materials.length > 0 && receipt.status === 'confirmed'
      ) || [];
    }, [receiptsImport]);
