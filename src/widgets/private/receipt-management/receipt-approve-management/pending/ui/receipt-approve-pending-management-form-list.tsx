...
const headCellsProduct = [
      ...
      { id: 'actions', label: 'Thao tác', sortable: true }
    ];
...
const ReceiptImportManagementFormListMaterial = ({receipts, users, suppliers, specifications, fetchAllReceipt}: {fetchAllReceipt?: () => void; receipts:ReceiptData[]; users: UserData[], suppliers: SupplierData[], specifications?: Specification[]} ) => {
...
return (
    <Box sx={{ width: '100%' }}>
      ...
      <Dialog
        open={isShowMaterial !==null || isApprove !== null}
...
const ReceiptImportManagementFormListProduct = ({receipts, users, specifications, fetchAllReceipt}: {fetchAllReceipt?: () => void;receipts:ReceiptData[]; users: UserData[], specifications?: Specification[]}) => {
    const [page, setPage] = useState(0);
    const [rowsPerPage, setRowsPerPage] = useState(10);
    const [searchTerm, setSearchTerm] = useState('');
    const [sortBy, setSortBy] = useState<string>('name');
...
return (
    <Box sx={{ width: '100%' }}>
      ...
      {/* Dialog hiển thị sản phẩm */}
            <Dialog
              open={isShowProduct !==null || isApprove !==null}
              onClose={handleCloseDialog}
              aria-labelledby="product-dialog-title"
              aria-describedby="product-dialog-description"
              maxWidth="lg"
...
const ReceiptExportManagementFormListMaterial = ({receipts, users, suppliers, specifications, fetchAllReceipt}: {fetchAllReceipt?: () => void;receipts:ReceiptData[]; users: UserData[], suppliers: SupplierData[], specifications?: Specification[]}) => {
    const [page, setPage] = useState(0);
    const [rowsPerPage, setRowsPerPage] = useState(10);
    const [searchTerm, setSearchTerm] = useState('');
    const [sortBy, setSortBy] = useState<string>('name');
...
const ReceiptExportManagementFormListProduct = ({receipts, users,specifications, fetchAllReceipt}: {fetchAllReceipt?: () => void;receipts:ReceiptData[]; users: UserData[], specifications?: Specification[]}) => {
    const [page, setPage] = useState(0);
    const [rowsPerPage, setRowsPerPage] = useState(10);
    const [searchTerm, setSearchTerm] = useState('');
    const [sortBy, setSortBy] = useState<string>('name');
...
export const ReceiptApprovePendingManagementFormList = () => {
    const theme = useTheme();
    ...
    const receiptsImportProduct = useMemo(() => {
      return receiptsImport?.filter((receipt) => 
        receipt.products && receipt.products.length > 0 && receipt.status === 'pending'
      ) || [];
    }, [receiptsImport]);
    const receiptsImportMaterial = useMemo(() => {
      return receiptsImport?.filter((receipt) => 
        receipt.materials && receipt.materials.length > 0 && receipt.status === 'pending'
      ) || [];
    }, [receiptsImport]);
    ...
    const getInfoTab = () => {
        switch (tabIndex) {
            case 0:
                return (
                    <ReceiptImportManagementFormListMaterial suppliers={suppliers} receipts={receiptsImportMaterial} users={users} specifications={specifications} fetchAllReceipt={fetchAllReceipts}/>
                );
            case 1:
                return (
                    <ReceiptImportManagementFormListProduct receipts={receiptsImportProduct} users={users} specifications={specifications} fetchAllReceipt={fetchAllReceipts}/>
                );
            case 2:
                return (
                    <ReceiptExportManagementFormListMaterial suppliers={suppliers} receipts={receiptsExportMaterial} users={users} specifications={specifications as Specification[]} fetchAllReceipt={fetchAllReceipts} />
                );
            case 3:
                return (
                    <ReceiptExportManagementFormListProduct receipts={receiptsExportProduct} users={users} specifications={specifications} fetchAllReceipt={fetchAllReceipts}/>
                );
            default:
                return null;
        }
