...
const ReceiptExportManagementFormListMaterial = ({receipts, users, suppliers, fetchAllReceipt, specifications}: {receipts:ReceiptData[]; fetchAllReceipt: () => void; users: UserData[], suppliers: SupplierData[], specifications?: Specification[]}) => {
        ...
        <Box
          ...
          >
             <Box sx={{p:1, backgroundColor: theme.palette.primary.main, display: 'flex', alignItems: 'center'}}>
                      <Link href='/admin/receipt-management/export/add/material' style={{textDecoration: 'none', display: 'flex', alignItems: 'center',  color: theme.palette.text.secondary, cursor: 'pointer' }}>
                        <Add sx={{fontSize: theme.typography.fontSize}}/> Tạo phiếu xuất kho
                      </Link>
                </Box>
            {isIndeterminate && (
                      ...
                      <TableCell>
                        {/* Hành động */}
                            <IconButton 
                                ...
                                size='small'
                            >
                                <Link href={`/admin/receipt-management/export/edit/material/${item._id}`} style={{color: theme.palette.success.main}}>
                                  <Edit/>
                                </Link>
                            </IconButton>
                            <IconButton 
...
export const ReceiptExportManagementFormListProduct = ({receipts, users, fetchAllReceipt, specifications}: {receipts:ReceiptData[]; fetchAllReceipt: () => void; users: UserData[], specifications?: Specification[]}) => {
    const [page, setPage] = useState(0);
    const [rowsPerPage, setRowsPerPage] = useState(10);
    const [searchTerm, setSearchTerm] = useState('');
    const [sortBy, setSortBy] = useState<string>('name');
    ...
    <Box sx={{ width: '100%' }}>
      ...
      <Paper sx={{ mb: 2, p: 2, borderRadius: 0, backgroundColor: theme.palette.background.default }}>
        <Box
          ...
          >
             <Box sx={{p: 1, backgroundColor: theme.palette.primary.main, display: 'flex', alignItems: 'center'}}>
                      <Link href='/admin/receipt-management/export/add/product' style={{textDecoration: 'none', display: 'flex', alignItems: 'center',  color: theme.palette.text.secondary, cursor: 'pointer' }}>
                        <Add sx={{fontSize: theme.typography.fontSize}}/> Tạo phiếu xuất kho
                      </Link>
                </Box>
            {isIndeterminate && (
                      ...
                      <TableCell>
                        {/* Hành động */}
                            <IconButton 
                                ...
                                size='small'
                            >
                                <Link href={`/admin/receipt-management/export/edit/product/${item._id}`} style={{color: theme.palette.success.main}}>
                                  <Edit/>
                                </Link>
                            </IconButton>
                            <IconButton 
...
export const ReceiptExportManagementFormList = () => {
    ...
    const fetchAllReceipts = async () => {
        ...
        if(response.success) {
          setReceipts(response.data?.filter(el => el.typeReceipt === 'export') || []);
        }
      }
    ...
    const getInfoTab = () => {
        switch (tabIndex) {
            case 0:
                return (
                    <ReceiptExportManagementFormListMaterial suppliers={suppliers} receipts={receiptsMaterial} fetchAllReceipt={fetchAllReceipts} users={users} specifications={specifications as Specification[]} />
                );
            case 1:
                return (
                    <ReceiptExportManagementFormListProduct receipts={receiptsProduct} fetchAllReceipt={fetchAllReceipts} users={users} specifications={specifications}/>
                );
            default:
                return null;
        }
