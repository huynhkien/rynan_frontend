'use client'
import { updateProductReceipt } from "@/features/receipt/api/receiptApi";
...
export const ReceiptImportManagementFormAddEditProductItem = ({materialId, receipt, render, isEditProductState, products, productReceipt, handleSelectProduct, product, specifications}: ReceiptImportManagementFormAddEditProductItemProps) => {
    ...
    const handleAddProduct = async () => {
        ...
        const existedProduct = receipt?.products?.find(el => el.pid === product._id);
        if (existedProduct) {
            const isMismatch = 
                existedProduct.batchNumber !== selectedBatchNumber ||
    ...
    return (
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, p: 2 }}>
            ...
            />
            {materialId ?
            <ButtonItem onClick={handleUpdateProductById} sx={{py:2, color: theme.palette.text.secondary, backgroundColor: theme.palette.primary.main}}>
                Cập nhật sản phẩm
            </ButtonItem>
            :
