...
const ReceiptExportPDFDocument = ({ 
  ...
  <Document>
    <Page size="A4" style={styles.page}>
      ...
      <Text style={styles.title}>{receipt.products && receipt?.products?.length > 0 ? 'PHIẾU NHẬP KHO SẢN PHẨM' : 'PHIẾU NHẬP KHO NGUYÊN LIỆU' }</Text>
      {/* Thông tin nhân viên */}
        <View style={styles.infoSection}>
          <View style={styles.infoLeft}>
            <View style={styles.infoRow}>
              <Text style={styles.label}>Nhân viên thực hiện:</Text>
              <Text style={styles.value1}>{users.find(el => el._id === receipt.staff)?.name || ''} </Text>
            </View>
            <View style={styles.infoRow}>
              <Text style={styles.label}>Chức vụ:</Text>
              <Text style={styles.value}>{Role.find(el => el._id === users.find(el => el._id === receipt.staff)?.role)?.name || ''}</Text>
            </View>
            <View style={styles.infoRow}>
              <Text style={styles.label}>Số điện thoại:</Text>
              <Text style={styles.value}>{users.find(el => el._id === receipt.staff)?.phone || ''}</Text>
            </View>
            <View style={styles.infoRow}>
              <Text style={styles.label}>Email:</Text>
              <Text style={styles.value}>{users.find(el => el._id === receipt.staff)?.email || ''}</Text>
            </View>
          </View>
        ...
        {/* Bảng sản phẩm */}
        <Text style={styles.sectionTitle}>1. Thông tin sản phẩm</Text>
        ...
        {receipt.materials && receipt?.materials.map((material, index) => (
          <View style={styles.tableRow} key={material.mid}>
            ...
            <View style={styles.tableCol41}>
              <Text style={styles.tableCellLeft}>{material?.price|| ''}</Text>
            </View>
      ...
      {/* Tóm tắt đơn hàng */}
        <View>
          ...
          <Text style={styles.noteItem}>
            ...
            <Text style={styles.boldText}>          Rynan Smart Agriculture</Text>
          </Text>
...
export const ReceiptImportManagementFormExport = ({ 
  users,
  receipt,
  specifications,
  suppliers
