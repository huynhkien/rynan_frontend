...
import { updateInfoByUser } from "@/features/user/api/userApis";
...
import { updateUserSuccess } from "@/features/user/store/userSlice";
...
export const UserInfoManagementFormEdit = () => {
    ...
    useEffect(() => {
            ...
            try {
                if (current) {
                    reset({
                        ...
                        phone: current.phone || '',
                        email: current.email || '',
                        identification_card: current.identification_card || '',
                        website: current.website || '',
                        dateOfBirth: (current.dateOfBirth as string)?.split('T')[0] || '', 
                        gender: current.gender || '',
                        tax_code: current.tax_code || '',
                        invoice_address: current.invoice_address || '',
                        note: current.note || '',
                    });
                    setPreview(current.avatar?.url || '');
                    
                    // Xử lý địa chỉ theo chuỗi
                    if (current.address) {
                        // Step 1: Set Province
                        if (current.address.province?.code) {
                            setValue('address.province.code', current.address.province.code);
                            setTimeout(async () => {
                                await loadDistricts(current.address?.province.code as number);
                                if (current.address?.district.code as number) {
                                    setValue('address.district.code', current.address?.district.code as number);
                                    setTimeout(async () => {
                                        await loadWards(current.address?.district.code as number);
                                        if (current.address?.ward.code as number) {
                                            setValue('address.ward.code', current.address?.ward.code as number);
                                        }
                                    }, 100);
                                }
                            }, 100);
                        }
                        if (current.address.detail) {
                            setValue('address.detail', current.address.detail);
                        }
                        if (current.address.addressAdd) {
                            setValue('address.addressAdd', current.address.addressAdd);
                        }
                    }
                }
            } catch (error) {
        ...
        fetchClient();
    }, [current, reset, setValue, loadDistricts, loadWards ]);
    ...
    const handleUpdateUser = async (data: UserDataProps) => {
        ...
        try {
            ...
            if (response.success) {
                ...
                dispatch(updateUserSuccess(dataUpdate));
